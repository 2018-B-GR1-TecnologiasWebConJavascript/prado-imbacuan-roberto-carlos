...
Transformaciones['jsonAtexto'] = (contenido: any) => {
...
Transformaciones['textTojson'] = (contenido: string) => {
...
Transformaciones['listToTable'] = (lista: any[]) => {
    const t = new Table;
    lista.forEach((item) => {
        t.cell('ID', item.id)
...
Transformaciones['textToList'] = (texto: string) => {
...
Transformaciones['listToText'] = (lista: any[]) => {
    let texto = "";
    lista.forEach((item) => {
...
Transformaciones['keyEnToEsp'] = (key: string) => {
   let nueva_key = "";
    switch (key) {
        case 'Marca':
            nueva_key = 'brand';

...
examen$
    .pipe(map((data) => {
    let resouesta1 = data.bdd.map((value) => {
        return value['types'].map((slot) => slot.type.name);
    });
    const respuesta1_1 = [];
    resouesta1.forEach((lista) => {
        lista.forEach((texto) => {
    ...
    let resouesta2 = data.bdd.map((value) => {
        return value['abilities'].map((slot) => slot.ability.name);
    });
    const respuesta2_2 = [];
    resouesta2.forEach((lista) => {
        lista.forEach((texto) => {
    ...
    let resouesta3 = data.bdd.map((value) => {
        return value['moves'].map((slot) => slot.move.name);
    });
    const respuesta3_3 = [];
    resouesta3.forEach((lista) => {
        lista.forEach((texto) => {
    ...
    const types = data.preguntas[0].respuesta;
    const respueesta4 = [];
    types.forEach((type) => {
        const pokemons = [];
    ...
    const abilities = data.preguntas[1].respuesta;
    const respueesta5 = [];
    abilities.forEach((abilitie) => {
        const pokemons = [];
        data.bdd.forEach((pokemon) => {
            const a = [];
            const abilities_p = pokemon.abilities;
    ...
    const moves = data.preguntas[2].respuesta;
    const respueesta6 = [];
    moves.forEach((move) => {
        const pokemons = [];
        data.bdd.forEach((pokemon) => {
            const m = [];
            const moves_p = pokemon.moves;
